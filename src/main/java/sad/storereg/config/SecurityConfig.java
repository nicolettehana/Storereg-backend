...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		.authorizeHttpRequests(auth -> auth .requestMatchers("/auth/**", "/csrf-token","/api/**","/my-report","/api2").permitAll() 
				...
				.requestMatchers(GET, "/menu","/status","/year-range","/items/**","/category/**","/firms/**","/unit/**","rates/**","/purchase/**","/issue/**","/stock/**") .hasAnyAuthority(ADMIN.name(), CH.name(), USER.name(), EST.name(), SAD.name()) 
				...
				.requestMatchers(POST, "/firms/**","/items","/rates","/purchase/create","/issue/create").hasAnyAuthority(SAD.name())
