...
//	            )
		.authorizeHttpRequests(auth -> auth .requestMatchers("/auth/**", "/csrf-token","/api/**","/my-report","/api2").permitAll() 
				...
				.requestMatchers(GET, "/menu","/status","/year-range","/items/**","/category/**","/firms/**") .hasAnyAuthority(ADMIN.name(), CH.name(), USER.name(), EST.name(), SAD.name()) 
				...
				.requestMatchers(POST, "/firms","/items").hasAnyAuthority(SAD.name())
