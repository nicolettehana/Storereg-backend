...
//	    );
    ...
    (
        COALESCE(
            (
                SELECT SUM(
                    pi.quantity * COALESCE(r.base_unit_quantity,1)
                )
                FROM appdata.purchase_items pi
                JOIN appdata.purchase p
                    ON p.id = pi.purchase_id
                LEFT JOIN master.rates r
                    ON r.item_id = pi.item_id
                    AND COALESCE(r.sub_item_id,-1)
                        = COALESCE(pi.sub_item_id,-1)
                    AND r.unit = pi.unit_id
                WHERE pi.item_id = :itemId
                  AND COALESCE(pi.sub_item_id,-1)
                        = COALESCE(:subItemId,-1)
                  AND p.receipt_entrydate <= :issueDate
            ),
            0
        )
        ...
        COALESCE(
            (
                SELECT SUM(
                    ii.quantity * COALESCE(r.base_unit_quantity,1)
                )
                FROM appdata.issue_items ii
                JOIN appdata.issue i
                    ON i.id = ii.issue_id
                LEFT JOIN master.rates r
                    ON r.item_id = ii.item_id
                    AND COALESCE(r.sub_item_id,-1)
                        = COALESCE(ii.sub_item_id,-1)
                    AND r.unit = ii.unit_id
                WHERE ii.item_id = :itemId
                  AND COALESCE(ii.sub_item_id,-1)
                        = COALESCE(:subItemId,-1)
                  AND i.date <= :issueDate
            ),
            0
...
FROM master.items it
JOIN master.units bu
    ON bu.id = it.base_unit
